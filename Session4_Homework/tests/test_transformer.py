...
def sample_churn_data():
    ...
    return pd.DataFrame({
        ...
        "Gender": ["Female", "Female", "Female", "Female", "Female"], # Keeping same to test OneHotEncoder drop='first' behavior cleanly
...
def test_transformer(sample_churn_data):
    ...
    assert "Geography" not in transformed_df.columns, "Original Geography column not dropped"
    ...
    assert "Geography_Spain" in transformed_df.columns, "Missing encoded column: Geography_Spain"
